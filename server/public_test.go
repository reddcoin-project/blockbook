...
func setupPublicHTTPServer(parser bchain.BlockChainParser, chain bchain.BlockChain, t *testing.T, extendedIndex bool) (*PublicServer, string) {
	...
	s, err := NewPublicServer("localhost:12345", "", d, chain, mempool, txCache, "", metrics, is, fiatRates, false, 30*time.Second, 60*time.Second, 120*time.Second, 10*time.Second, 45*time.Second)
	if err != nil {
		t.Fatal(err)
	}
	return s, path
