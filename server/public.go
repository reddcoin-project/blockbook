...
func (s *PublicServer) apiMoneySupply(r *http.Request, apiVersion int) (string, error) {
	sysInfo, err := s.api.GetSystemInfo(false)
	if err != nil {
		glog.Error(err)
		return "error", err
	}
	return fmt.Sprintf(sysInfo.Backend.MoneySupply), nil
}

func (s *PublicServer) apiTotalSupply(r *http.Request, apiVersion int) (string, error) {
	sysInfo, err := s.api.GetSystemInfo(false)
	if err != nil {
		glog.Error(err)
		return "error", err
...
func (s *PublicServer) apiBlockheight(r *http.Request, apiVersion int) (string, error) {
	sysInfo, err := s.api.GetSystemInfo(false)
	if err != nil {
		glog.Error(err)
		return "error", err
	}
	return fmt.Sprintf("%d", sysInfo.Backend.Blocks), nil
}
